...
function onCreate()
    ...
    makeLuaSprite('flash', 'white', -1000, -300)
...
function onCreatePost()
    ...
    setProperty('timeTxt.alpha', 0.1)
    setProperty('timeBarBG.alpha', 0.1)
    setProperty('timeBar.alpha', 0.1)
...
function onUpdate()
    ...
    if curStep == 128 then -- text goes away, camhud comes in. i forgot what i intended to do with camera tbh so i commented it out
        ...
        cameraFlash('game', getColorFromHex('0xffffff'), 1.0, false)
        for i = 0,7 do
    ...
    if curStep == 1320 or curStep == 1336 then 
        setProperty('camGame.alpha', 0)
        runTimer('Holis', 0.1)
    end

    if curStep == 1343 then -- vintage blends in with character, looks pretty cool. 'wait a second, this sounds familiar to me..' oh, yeah and also a bunch of things being hidden
        drain = 0
        setProperty('camGame.alpha', 0)
        runTimer('Reveal', 0.1)
        setProperty('healthBar.alpha', 0)
    ...
    if curStep == 2384 or curStep == 2512 or curStep == 2640 or curStep == 2832 then
        cameraFlash('game','FFFFFF',0.5)
    setProperty('pico.alpha', 0)
    setProperty('boyfriend.alpha', 0)
    setProperty('stageback.alpha', 0)
    ...
    if curStep == 2448 or curStep == 2576 or curStep == 2704 then
        cameraFlash('game','FFFFFF',0.5)
        setProperty('pico.alpha', 1)
        setProperty('boyfriend.alpha', 1)
        setProperty('stageback.alpha', 1)
...
function onTimerCompleted(tag, loops, loopsLeft)
    ...
    if tag == 'Reveal' then 
        doTweenAlpha('goodbye','camGame',1,5,'EaseInOut')
    end
end